...
async function run(configPath, resourcesDirectory, sources, resources) {
    ...
    if (resources.find(resource => resource.type === "adaptive-icon" /* ADAPTIVE_ICON */)) {
...
function runResource(configPath, resource, container) {
    ...
    if (typeof src !== 'string') {
        throw new error_1.BadInputError(`Bad value for src key: ${resource.srckey}`);
    }
    // We force the use of forward slashes here to provide cross-platform
    // compatibility for paths.
    const value = path_1.default.relative(path_1.default.dirname(configPath), src).replace(/\\/g, '/');
...
function resolveResourceElement(container, nodeName, pathAttr, pathAttrValue) {
